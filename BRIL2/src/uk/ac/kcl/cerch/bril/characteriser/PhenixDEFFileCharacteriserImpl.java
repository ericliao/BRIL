...
public class PhenixDEFFileCharacteriserImpl implements
		PhenixDEFFileCharacteriser {
	...
	public FileCharacterisation characteriseFile(File file)
		...
		fileCharacterisation = new PhenixDEFFileCharacterisation();

		DEFPhenixProcessor phenixProcessor = new DEFPhenixProcessor(file);
		TaskObject taskObject = phenixProcessor.getTaskObject();
	...
	private String runXMLCreation(TaskObject taskObject) throws Exception {
		...
		writer.writeAttribute(TaskObjectElement.JOB_ID.localName(), taskObject
				.getJobID());

		writer.writeStartElement(task_dataURI, TaskObjectElement.TASK_NAME
				.localName());
	...
	/*	writer.writeStartElement(task_dataURI, TaskObjectElement.DATE
		...
		writer.writeStartElement(task_dataURI, TaskObjectElement.SOFTWARE_NAME
				.localName());
		...
		// here
		writer.writeStartElement(task_dataURI, TaskObjectElement.INPUT_FILENAME
				.localName()
				+ "s");
